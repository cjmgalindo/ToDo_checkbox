...
document.querySelector("#createdBy").innerHTML += " John";
...
//el objetivo de la funcion es cambiarle el this.complete depedndiendo si la tarea ya esta realizada o no.
...
function buildToDo(todo, index) {
  /*    Investigá sobre el tipo 'checkbox' del elemento input y realizar lo siguiente en la función 'buildToDo':
        ...
        d) Asignarle la clase '' al checkbox
        ...
        toDoShell.appendChild(toDoText000)
*/
  // let checkbox = document.qusetAttribute("type","checkbox")
  // checkbox.id = index;

  let toDoShell = document.createElement("div"); // crea un elemento (div)
  toDoShell.className = "toDoShell"; // le agregamos clase al div.

  let toDoText = document.createElement("span"); //crea un elemento span

  //practicar
  toDoText.innerHTML = todo.description; //ingresa contenido al span
  // toDoText.id = index                       //asigna un id al span (CSS)
  ...
  cBox.classList = "completeCheckbox";
  ...
  console.log(cBox.getAttribute("checked"))
  toDoShell.appendChild(toDoText);
  //mas adelante lo pide una fn
  // toDoText.addEventListener("click", completeToDo)

  return toDoShell;
  // <div class='toDoShell'>  <span id=0>  </span> </div>
}
...
function displayToDos() {
  ...
  let newTodo = buildToDos(toDoItems);

  for (let i = 0; i < newTodo.length; i++) {
    toDoContainer.appendChild(newTodo[i]);
  }
}
